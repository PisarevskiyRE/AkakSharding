...
/*
Небольшой платежный сервис - ЦЕНТРАЛИЗОВАННЫЙ
 - поддержание четко определенного порядка транзакций
...
class PaymentSystem extends Actor with ActorLogging {
  override def receive: Receive = {
    case t: Transaction => log.info(s"Проверка транзакций $t") // добавьте сюда сложную бизнес-логику
    case m => log.info(s"Другое: $m")
  }
}
...
class PaymentSystemNode(port: Int, shouldStartSingleton: Boolean = true) extends App {
  val config = ConfigFactory.parseString(
    s"""
     ...
     """.stripMargin)
    .withFallback(ConfigFactory.load("part4_adv/singleton.conf"))
