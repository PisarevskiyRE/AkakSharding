...
class WordCountMaster extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case Initialize(nWorkers) =>
        ...
        workerSelection.foreach(_ ! Identify("start"))
        context.become(initializing(List(), nWorkers))
      /*
        TODO определить акторы на удаленных станциях
        - создать акторы от 1 до nWorkers
        - отправить сообщение идентификатора всем акторам
        - получить состояние всех ответивших аторов
       */
  }

  def initializing(workers: List[ActorRef], remainingWorkers: Int): Receive = {
    case ActorIdentity("start", Some(workerRef)) =>
